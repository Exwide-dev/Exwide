...
class Parser:
    ...
    def _consume_statement_end(self):
        ...
        if self._is_valid():
            token = self._current_token()
            if token.typ in ['SEMICOLON', 'ENDL']:
                self._advance()  # 跳过分号或换行符
    ...
    def _parse_return_statement(self) -> Dict[str, Any]:
        ...
        if self._is_valid() and self._current_token().typ not in ['SEMICOLON', 'ENDL', 'RBRACE']:
    ...
    def _parse_block(self) -> List[Dict[str, Any]]:
        ...
        while self._is_valid() and self._current_token().typ != 'RBRACE':
            # 在代码块内，换行符和分号都作为语句分隔符
            if self._current_token().typ in ['ENDL', 'SEMICOLON']:
                self._advance()
                continue
    ...
    def _parse_operator_expression(self) -> Optional[Dict[str, Any]]:
        ...
        clog(f'运行_parse_operator_expression, 左边为: {left}')
        ...
        clog(
            ...
            self._current_token().typ == 'OPERATOR': {self._current_token().typ == 'OPERATOR'},
            self._current_token().val in operators: {self._current_token().val in operators}
            '''
        )
